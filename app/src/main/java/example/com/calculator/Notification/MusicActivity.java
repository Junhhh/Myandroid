...
public class MusicActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(ContextCompat.checkSelfPermission(MusicActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(MusicActivity.this,new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, 2);
        }else {
            initMediaPlayer();
        }
    }
    ...
    private void initMediaPlayer() {
        ...
        try {
            File file = new File(Environment.getExternalStorageDirectory(),"music.mp3");//指定了根目录 music.mp3 这个文件
            mediaPlayer.setDataSource(file.getPath());//指定音乐文件路径
